# coding: utf-8
lib = File.expand_path('../lib', __FILE__)
$LOAD_PATH.unshift(lib) unless $LOAD_PATH.include?(lib)
require 'fv/version'

Gem::Specification.new do |spec|
  spec.name          = 'fv'
  spec.version       = FV::VERSION
  spec.authors       = ['FocusVision']
  spec.email         = ['portlanddevelopers@focusvision.com']

  spec.summary       = 'Common FocusVision utilities'
  spec.description   = 'The fv gem provides common tools for creating FV SDKs'
  spec.homepage      = 'https://invent.focusvision.com/Portland/fv'

  spec.metadata['allowed_push_host'] = 'http://gems.focusvision.com'

  spec.files         = `git ls-files -z`.split("\x0").reject { |f| f.match(%r{^(test|spec|features)/}) }
  spec.bindir        = 'exe'
  spec.executables   = spec.files.grep(%r{^exe/}) { |f| File.basename(f) }
  spec.require_paths = ['lib']

  spec.add_dependency 'httparty', '~> 0.14.0'
  spec.add_dependency 'activesupport'

  spec.add_development_dependency 'bundler', '~> 1.11'
  spec.add_development_dependency 'rake', '~> 10.0'
  spec.add_development_dependency 'rspec', '~> 3.0'
  spec.add_development_dependency 'pry-inline', '~> 1.0.2'
  spec.add_development_dependency 'webmock', '~> 2.3.1'
end
